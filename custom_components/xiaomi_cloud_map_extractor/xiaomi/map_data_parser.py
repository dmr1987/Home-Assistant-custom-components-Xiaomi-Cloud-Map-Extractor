...
class MapDataParserXiaomi(MapDataParser):
    ...
    def parse_image(block_data_length: int, block_header_length: int, data: bytes, header: bytes, carpet_map: Set[int],
                    colors: Colors, image_config: ImageConfig, 
                    bg_image_use: bool, bg_image_path: str, bg_image_alpha:int) -> Tuple[ImageData, Dict[int, Room]]:
        ...
        rooms = {}
        if bg_image_use:
            bg_image_layer = ImageHandlerXiaomi.load_bg_image(int(image.width), int(image.height), bg_image_alpha, bg_image_path)
